    ...
    return fetchFromBrockApi('all-events').then((apiResponse => {
        ...
        if ( ! conv.surface.capabilities.has('actions.capability.SCREEN_OUTPUT') ) {
            ...
            let speechEvents = events.map((event) => {
                return event['event_name'] + ` happening ` + moment.tz(event['start_datetime'], 'America/Toronto').calendar() + `.`
            })
        ...
        var eventCarouselItems = events.map((event) => {
            var descriptionText = moment.tz(event['start_datetime'], 'America/Toronto').calendar() + `  \n`
        ...
        conv.close(`Here are the most recent events.`)
